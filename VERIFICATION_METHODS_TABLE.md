# State Verification Methods Quick Reference

| State | Tier | ID? | Methods | Notes |
|-------|------|-----|---------|-------|
| Alabama | 1 | ✓ | CS | Commercially reasonable software (CC qualifies) - ... |
| Alaska | 0 | ✗ | - | No age verification laws currently enacted |
| Arizona | 2 | ✓ | DID, GID, TD | One-third threshold; very high penalties if minors... |
| Arkansas | 3 | ✓ | DID, GID, IAL2 | Requires IAL2 certification - very strict |
| California | 0 | ✗ | - | AB 1043 is a new law signed October 2025, which wi... |
| Colorado | 0 | ✗ | - |  |
| Connecticut | 0 | ✗ | - |  |
| Delaware | 4 | ✗ | - | This law seemingly applies only to providers of ad... |
| District of Columbia | 0 | ✗ | - |  |
| Florida | 3 | ✓ | Anon | Requires anonymous option; government enforcement |
| Georgia | 3 | ✓ | DID, GID, IAL2 | Requires IAL2; 1 year statute of limitations |
| Hawaii | 0 | ✗ | - |  |
| Idaho | 2 | ✓ | DID, GID, TD |  |
| Illinois | 0 | ✗ | - |  |
| Indiana | 2 | ✓ | TD, 3P |  |
| Iowa | 0 | ✗ | - |  |
| Kansas | 2 | ✓ | CDB | Very high private right of action damages ($50,000... |
| Kentucky | 2 | ✓ | GID, TD | Jury decides liability and damages |
| Louisiana | 2 | ✓ | DID, GID, TD | One-third threshold for substantial amount |
| Maine | 0 | ✗ | - |  |
| Maryland | 0 | ✗ | - |  |
| Massachusetts | 0 | ✗ | - |  |
| Michigan | 0 | ✗ | - |  |
| Minnesota | 0 | ✗ | - |  |
| Mississippi | 2 | ✓ | DID, GID, TD |  |
| Missouri | 2 | ✓ | GID, TD |  |
| Montana | 2 | ✓ | DID, GID, TD |  |
| Nebraska | 2 | ✓ | DID, GID, TD, FD, CS | Parent may recover actual damages |
| Nevada | 0 | ✗ | - |  |
| New Hampshire | 0 | ✗ | - |  |
| New Jersey | 0 | ✗ | - |  |
| New Mexico | 0 | ✗ | - |  |
| New York | 0 | ✗ | - |  |
| North Carolina | 2 | ✓ | CDB | Allows punitive damages |
| North Dakota | 2 | ✓ | DID, GID, TD |  |
| Ohio | 0 | ✗ | - |  |
| Oklahoma | 2 | ✓ | DID, TD, 3P | ISP blocking requirement |
| Oregon | 0 | ✗ | - |  |
| Pennsylvania | 0 | ✗ | - |  |
| Rhode Island | 0 | ✗ | - |  |
| South Carolina | 2 | ✓ | DID, TD, 3P, CDB | Punitive damages when minor affected |
| South Dakota | 1 | ✓ | CC, BA, GID | Explicitly allows credit/debit cards - Tier 1 |
| Tennessee | 4 | ✓ | TD, PM | Photo matching requirement; 7-year data retention |
| Texas | 2 | ✓ | DID, GID, TD | High penalties; $250k if minors access |
| Utah | 2 | ✓ | DID, TD, 3P |  |
| Vermont | 0 | ✗ | - |  |
| Virginia | 2 | ✓ | CDB |  |
| Washington | 0 | ✗ | - |  |
| West Virginia | 0 | ✗ | - |  |
| Wisconsin | 0 | ✗ | - |  |
| Wyoming | 1 | ✓ | CC, GID | Explicitly allows credit cards (18+ required) - Ti... |

## Legend:
- CC = Credit Card
- DID = Digitized ID
- GID = Government ID
- TD = Transactional Data
- FD = Financial Document
- IAL2 = Identity Assurance Level 2
- PM = Photo Matching
- Anon = Anonymous Option
- 3P = Third Party Service
- CDB = Commercial Database
- CS = Commercially Reasonable Software
- BA = Bank Account
